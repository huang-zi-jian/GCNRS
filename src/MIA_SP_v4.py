...
class SGcn(nn.Module):
    ...
    def forward(self):
        # user_structure_list = [self.user_structure]
        # item_structure_list = [self.item_structure]
        user_structure_list = [func.normalize(self.user_structure, p=2, dim=-1)]
        item_structure_list = [func.normalize(self.item_structure, p=2, dim=-1)]
        ...
        for layer in range(self.flags_obj.n_layers - 2):
            ...
            # user_structure_list.append(graph_droped @ item_structure_list[layer])
            # item_structure_list.append(graph_droped.T @ user_structure_list[layer])

            user_structure_list.append(func.normalize(graph_droped @ item_structure_list[layer], p=2, dim=-1))
            item_structure_list.append(func.normalize(graph_droped.T @ user_structure_list[layer], p=2, dim=-1))

        user_structure_merge = torch.stack(user_structure_list, dim=-1)
        user_structure_merge = torch.mean(user_structure_merge, dim=-1)

        item_structure_merge = torch.stack(item_structure_list, dim=-1)
        item_structure_merge = torch.mean(item_structure_merge, dim=-1)

        return user_structure_merge, item_structure_merge
