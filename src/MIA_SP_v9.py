...
class MIA(nn.Module):
    ...
    def structure_loss(self, users_structure, weak_items_structure, strong_items_structure):
        ...
        weak_strong_structure_loss = torch.mean(func.relu(torch.sub(weak_structure_scores, strong_structure_scores - 0.1)))
    ...
    def fusion_sample(self, users, adjacent_items, items_pool, items_weight, users_preference, items_preference,
        ...
        adj_embed = torch.cat((items_preference[adjacent_items], items_structure[adjacent_items]), dim=-1)
        update_weight = []
        for i in range(items_pool.shape[1]):
            ...
            #     0.99 * torch.sum(torch.mul(users_structure[users], items_structure[items]), dim=-1) + 0.01 * torch.sum(
    ...
    def forward(self, users, adjacent_items, items_pool, items_weight):
        ...
        negative_items = self.fusion_sample(users, adjacent_items, items_pool, items_weight, users_preference,
                                            items_preference, users_structure, items_structure)
