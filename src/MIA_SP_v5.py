...
class MIA(nn.Module):
    ...
    def forward(self, users, adjacent_items, items_pool, items_weight):
        ...
        structure_loss = self.structure_loss(users_structure_embed, adj_structure_embed,
                                             weak_structure_embed, strong_structure_embed)
        ...
        adj_decision_embed = self.item_decision[adjacent_items]
        strong_decision_embed = self.item_decision[update_strong_items]
        ...
        loss = bpr_loss + preference_loss + 0.01 * structure_loss
...
class Structure(nn.Module):
    ...
    def computer(self, users, adjacent_items):
        sample_scr = sp.csr_matrix((np.ones(users.shape[0]), (users.cpu(), adjacent_items.cpu())), shape=(self.num_users, self.num_items),
                                   dtype=np.int16)
